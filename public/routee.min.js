...
// EXPORTS
__webpack_require__.d(__webpack_exports__, "routee", function() { return /* reexport */ routeeV2_routee; });

// EXTERNAL MODULE: ./node_modules/routeemodal/src/css/routeemodal.css
var routeemodal = __webpack_require__(7);

// EXTERNAL MODULE: ./src/css/routee.css
var routee = __webpack_require__(8);

// CONCATENATED MODULE: ./src/js/constants.js
/* harmony default export */ var constants = ({
    URL: {
        sw: '/routee-sw.js',
        base: 'https://analytics-service.routee.net',
        frame: 'https://analytics-service.routee.net/rcc/index.php',
        optin: 'https://analytics-service.routee.net/development/push',
        config: 'https://analytics-service.routee.net/development/config',
        session: 'https://analytics-service.routee.net/development/session',
        icon: 'https://cdn0.routee.net/resources/img/notification_icon.svg'
    },
    COOKIES: {
        analytics: 'routee_analytics',
        tracking: 'routee_cookie',
        session: 'routee_session',
        countSessions: 'routee_cnt',
        later: 'pushLater'
    },
    CSS: {
        allowBtnCls: 'push-notification-allow',
        disallowBtnCls: 'push-notification-later'
    },
    CONDITIONS: {
        TYPES: {
...
class Utils_Utils {
    ...
    /**
     * Encode a number with an UUID seed
     *
     * @param {number} value
     * @param {string} uuid
     * @return {string} hexadecimal
     */
    static encodeNumber(value, uuid) {
        let seed = this._uuidToInt(uuid);
        let final = seed + value;
        return final.toString(16);
    }

    /**
     * Decode a number with an UUID seed
     *
     * @param {string} value - hexadecimal
     * @param {string} uuid
     * @return {number}
     */
    static decodeNumber(value, uuid) {
        let seed = this._uuidToInt(uuid);
    ...
    /**
     * Convert UUID to decimal number
     *
     * @param {string} uuid
     * @return {number}
     * @private
     */
    static _uuidToInt(uuid) {
        let hexArray = uuid.split("-");
        let intUuid = 0;
        hexArray.forEach(hex => intUuid += parseInt(hex, 16));
        return intUuid;
    }
    ...
    static getBrowserData() {
        let analyticsId = Cookie_Cookie.get(constants.COOKIES.analytics);
        ...
        data.timestamp = dateNow.getTime() / 1000 | 0;
        data.timezoneOffset = dateNow.getTimezoneOffset() / 60;
        data.cookiesEnabled = navigator.cookieEnabled;
        data.characterSet = document.characterSet;
        data.resolution = {
...
class Cookie_Cookie {
     ...
     * @param {int} [seconds=0] - After how many seconds the cookie will expire, if seconds<=0 then it expires on 2040-1-1
     */
    static set(name, value, seconds = 0) {
        let e = (seconds <= 0) ? new Date('1 Jan 2040 00:00:00 GMT').toUTCString() : new Date(Date.now() + (seconds * 1000)).toUTCString();
        document.cookie = name + '=' + value + ';expires=' + e + ';path=/;SameSite=None; Secure';
    }
     ...
     */
    static setSession() {
        document.cookie = constants.COOKIES.session + '=' + '_s' + ';path=/;SameSite=None; Secure';
        this._setSessionCount();
    }

    /**
     * Set cookie to count sessions
     *
     * @private
     */
    static _setSessionCount() {
        const sessionCount = this.getSessionCount();
        const encodedSessionCount = Utils_Utils.encodeNumber(sessionCount + 1, constants.ROUTEE_TOKEN);
        this.set(constants.COOKIES.countSessions, encodedSessionCount);
    }
    ...
    static getSessionCount() {
        const sessionCount = this.get(constants.COOKIES.countSessions);
        return (sessionCount !== null) ? Utils_Utils.decodeNumber(sessionCount, constants.ROUTEE_TOKEN) : 0;
    }
     ...
     */
    static getSession() {
        return this.get(constants.COOKIES.session) !== null;
    }
}
...
var index_cjs = __webpack_require__(6);
...
var component_dist_index_cjs = __webpack_require__(2);
...
function registerInstallations(instance) {
    ...
    instance.INTERNAL.registerComponent(new component_dist_index_cjs["Component"](installationsName, function (container) {
...
registerInstallations(dist_index_cjs_default.a);
...
function logEvent(gtagFunction, analyticsId, eventName, eventParams, options) {
    ...
    if (!options || !options.global) {
        params = Object(tslib_es6["__assign"])(Object(tslib_es6["__assign"])({}, eventParams), { 'send_to': analyticsId });
    }
...
var ERRORS = (dist_index_esm_a = {},
    dist_index_esm_a["no-ga-id" /* NO_GA_ID */] = "\"" + ANALYTICS_ID_FIELD + "\" field is empty in " +
        'Firebase config. Firebase Analytics ' +
        'requires this field to contain a valid measurement ID.',
    dist_index_esm_a["already-exists" /* ALREADY_EXISTS */] = 'A Firebase Analytics instance with the measurement ID ${id} ' +
        ' already exists. ' +
    ...
    dist_index_esm_a["interop-component-reg-failed" /* INTEROP_COMPONENT_REG_FAILED */] = 'Firebase Analytics Interop Component failed to instantiate',
    dist_index_esm_a);
...
function factory(app, installations) {
    ...
    if (!analyticsId) {
        throw index_esm_ERROR_FACTORY.create("no-ga-id" /* NO_GA_ID */);
    }
...
function registerAnalytics(instance) {
    instance.INTERNAL.registerComponent(new component_dist_index_cjs["Component"](ANALYTICS_TYPE, function (container) {
    ...
    instance.INTERNAL.registerComponent(new component_dist_index_cjs["Component"]('analytics-internal', internalFactory, "PRIVATE" /* PRIVATE */));
...
registerAnalytics(dist_index_cjs_default.a);
...
var ERROR_MAP = (messaging_dist_index_esm_a = {},
    ...
    messaging_dist_index_esm_a["token-subscribe-failed" /* TOKEN_SUBSCRIBE_FAILED */] = 'A problem occured while subscribing the user to FCM: {$errorInfo}',
    ...
    messaging_dist_index_esm_a["token-unsubscribe-failed" /* TOKEN_UNSUBSCRIBE_FAILED */] = 'A problem occured while unsubscribing the ' +
        'user from FCM: {$errorInfo}',
    messaging_dist_index_esm_a["token-update-failed" /* TOKEN_UPDATE_FAILED */] = 'A problem occured while updating the user from FCM: {$errorInfo}',
...
var dist_index_esm_ERROR_FACTORY = new util_dist_index_cjs["ErrorFactory"]('messaging', 'Messaging', ERROR_MAP);
    ...
    return Object(tslib_es6["__awaiter"])(this, void 0, void 0, function () {
        ...
        return Object(tslib_es6["__generator"])(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    tokenDetails = null;
                    return [4 /*yield*/, Object(idb["openDb"])(OLD_DB_NAME, OLD_DB_VERSION, function (db) { return Object(tslib_es6["__awaiter"])(_this, void 0, void 0, function () {
            ...
            return Object(tslib_es6["__generator"])(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 3:
                        ...
                        return [2 /*return*/, index_esm_getToken(this.firebaseDependencies, swRegistration, this.vapidKey)];
                }
            });
        });
    };
...
function getNotificationData(payload) {
    ...
    var notificationInformation = Object(tslib_es6["__assign"])({}, payload.notification);
    // Put the message payload under FCM_MSG name so we can identify the
    // notification as being an FCM notification vs a notification from
    // somewhere else (i.e. normal web push or developer generated
    // notification).
    notificationInformation.data = Object(tslib_es6["__assign"])(Object(tslib_es6["__assign"])({}, payload.notification.data), (_a = {}, _a[FCM_MSG] = payload, _a));
...
dist_index_cjs_default.a.INTERNAL.registerComponent(new component_dist_index_cjs["Component"](MESSAGING_NAME, factoryMethod, "PUBLIC" /* PUBLIC */).setServiceProps(NAMESPACE_EXPORTS));
...
// EXTERNAL MODULE: ./node_modules/es6-enum/dist/enum.js
var dist_enum = __webpack_require__(5);
var enum_default = /*#__PURE__*/__webpack_require__.n(dist_enum);

// CONCATENATED MODULE: ./node_modules/routeemodal/src/js/constants.js


const Enums = {
    ANIMATION_TYPE: enum_default()('none', 'slideUp', 'slideDown', 'slideLeft', 'slideRight', 'fade'),
    VERTICAL_POSITION: enum_default()('top', 'middle', 'bottom'),
    HORIZONTAL_POSITION: enum_default()('left', 'center', 'right'),
    CLOSE_TYPE: enum_default()('simple', 'classic', 'modern', 'none')
};

const Constants = {
    TRANSITION_CLASS: {
        slide: "transition-slide",
        slideUp: "transition-slide-out-down",
        slideDown: "transition-slide-out-up",
        slideLeft: "transition-slide-out-right",
        slideRight: "transition-slide-out-left",
        fade: "transition-fade",
        fadeOut: "transition-fade-out"
    },
    POPUP: {
        main: "routee-modal-container"
    }
};

// CONCATENATED MODULE: ./node_modules/routeemodal/src/js/ModalBuilder.js


/* harmony default export */ var ModalBuilder = (class {
     ...
     */
    _renderModal() {
        let modalBuilder = this;

        let modalBackgroundElement = document.createElement('div');

        if (modalBuilder._config.modal) {
            modalBackgroundElement.classList.add('routee-modal-background');
        } else {
            modalBackgroundElement.classList.add('routee-modal-no-background');
        }

        let modalContainerElement = document.createElement('div');
        modalContainerElement.classList.add(Constants.POPUP.main);
        modalContainerElement.style.setProperty('width', modalBuilder._config.width);
        modalContainerElement.style.setProperty('height', modalBuilder._config.height);

        if (modalBuilder._config.hasOwnProperty('type')) {
            switch (modalBuilder._config.type) {
                case Enums.CLOSE_TYPE.classic:
                    modalContainerElement.classList.add('routee-modal-type-3');
                    break;
                case Enums.CLOSE_TYPE.modern:
                    modalContainerElement.classList.add('routee-modal-type-2');
                    break;
                case Enums.CLOSE_TYPE.none:
                    modalContainerElement.classList.add('routee-modal-type-0');
                case Enums.CLOSE_TYPE.simple:
                default:
                    modalContainerElement.classList.add('routee-modal-type-1');
            }
        } else {
            modalContainerElement.classList.add('routee-modal-type-1');
        }

        modalBuilder._addAnimation(modalContainerElement, modalBuilder._config.animation);
        modalBackgroundElement.appendChild(modalContainerElement);

        let modalInnerElement = document.createElement('div');
        modalInnerElement.classList.add('routee-modal-inner', 'routee-modal-shadow');
        modalContainerElement.appendChild(modalInnerElement);

        let modalCloseElement = document.createElement('div');
        modalCloseElement.classList.add('routee-modal-close');
        let modalCloseXElement = document.createElement('span');
        modalCloseXElement.classList.add('routee-modal-x');
        modalCloseXElement.innerHTML = "×";
        modalCloseElement.appendChild(modalCloseXElement);

        if (modalBuilder._config.hasOwnProperty('header')) {
            let modalHeaderElement = document.createElement('div');
            modalHeaderElement.classList.add('routee-modal-header', 'routee-modal-justify');
            let modalHeaderTitleElement = document.createElement('div');
            modalHeaderTitleElement.classList.add('routee-modal-title');
            modalHeaderTitleElement.innerHTML = modalBuilder._config.header;
            modalHeaderElement.appendChild(modalHeaderTitleElement);
            modalHeaderElement.appendChild(modalCloseElement);
            modalInnerElement.appendChild(modalHeaderElement);
        } else {
            modalContainerElement.prepend(modalCloseElement)
        }

        let modalBodyElement = document.createElement('div');
        modalBodyElement.classList.add('routee-modal-body');
        modalInnerElement.appendChild(modalBodyElement);

        if (modalBuilder._config.hasOwnProperty('body')) {
            modalBodyElement.append(modalBuilder._config.body);
        } else {
            modalBodyElement.innerHTML = 'Hello World!';
        }

        if (modalBuilder._config.hasOwnProperty('footer')) {
            let modalFooterElement = document.createElement('div');
            modalFooterElement.classList.add('routee-modal-footer');
            modalFooterElement.append(modalBuilder._config.footer);
            modalInnerElement.appendChild(modalFooterElement);
        }

        modalBuilder._element = modalBackgroundElement;
    }

    /**
     * Add transition classes based on ANIMATION_TYPE
     *
     * @param {Element} element
     * @param {ANIMATION_TYPE} type
     * @private
     */
    _addAnimation(element, type) {
        switch (type) {
            case Enums.ANIMATION_TYPE.slideUp:
            case Enums.ANIMATION_TYPE.slideDown:
            case Enums.ANIMATION_TYPE.slideRight:
            case Enums.ANIMATION_TYPE.slideLeft:
                element.classList.add(Constants.TRANSITION_CLASS.slide);
                break;
            case Enums.ANIMATION_TYPE.fade:
                element.classList.add(Constants.TRANSITION_CLASS.fade);
                break;
            case Enums.ANIMATION_TYPE.none:
            default:
        }
    }
...
// CONCATENATED MODULE: ./node_modules/routeemodal/src/js/RouteeModal.js



/* harmony default export */ var RouteeModal = (class {

    /**
     * RouteeModal constructor
     *
     * @param {object} config
     * @param {number} [config.autoOpen] - Seconds till the popup auto-opens
     * @param {boolean} [config.modal=true] - If the popup will have an overly
     * @param {string} [config.appendTo=body]
     * @param {ANIMATION_TYPE} [config.animation]
     * @param {string} [config.width]
     * @param {string} [config.height]
     * @param {object} [config.position]
     * @param {HORIZONTAL_POSITION} config.position.horizontal
     * @param {VERTICAL_POSITION} config.position.vertical
     * @param {string} [config.position.horizontalMargin]
     * @param {string} [config.position.verticalMargin]
     * @param {string} [config.header]
     * @param {string|Element} [config.body]
     * @param {string|Element} [config.footer]
     * @param {CLOSE_TYPE} [config.type]
     */
    constructor(config) {
        // TODO: close on escape or click-out

        let routeeModal = this;
        routeeModal._setConfig(config);
        routeeModal._openEvent = function () {};
        routeeModal._closeEvent = function () {};
        routeeModal.isOpen = false;
        routeeModal._render();

        if (routeeModal._config.hasOwnProperty('autoOpen')) {
        ...
        function autoOpen() {
            setTimeout(function () {
                routeeModal.open();
            }, (routeeModal._config.autoOpen * 1000));
        }
    }
    ...
    open() {
        ...
        if (routeeModal.isOpen === false) {
            routeeModal._openEvent();
            routeeModal.element.style.display = 'block';

            if (routeeModal._config.modal === true) {
                document.querySelector(routeeModal._config.appendTo).style.setProperty('overflow', 'hidden', 'important');
            }

            setTimeout(function () {
                routeeModal._modalToggle();
                routeeModal.isOpen = true;
            }, 20);
        }
    }
    ...
    close() {
        ...
        if (routeeModal.isOpen === true) {
            routeeModal._closeEvent();
            let timeout = 0;

            if (routeeModal._config.animation === Enums.ANIMATION_TYPE.fade) {
                timeout = 800;
            } else if (routeeModal._config.animation !== Enums.ANIMATION_TYPE.none) {
                timeout = 1500;
            }

            setTimeout(function () {
                routeeModal.element.style.display = 'none';
            }, timeout);

            if (routeeModal._config.modal === true) {
                document.querySelector(routeeModal._config.appendTo).style.removeProperty('overflow');
                // TODO: store previous overflow if existed
            }
    ...
    destroy() {
        this.element.remove();
        delete this.element;
        this._openEvent = function () {};
        this._closeEvent = function () {};
        this.open = function () {};
        this.close = function () {};
    }

    /**
     * Event action when popup opens
     *
     * @param {function} callback
     */
    onOpen(callback) {
        this._openEvent = callback;
    }

    /**
     * Event action when popup closes
     *
     * @param {function} callback
     */
    onClose(callback) {
    ...
    /**
     * Parse, validate and set the config
     *
     * @param {object} config
     * @private
     */
    _setConfig(config) {
        ...
        if (config.hasOwnProperty('autoOpen')) {
            if (Number.isInteger(config.autoOpen) && config.autoOpen >= 0) {
                routeeModal._config.autoOpen = config.autoOpen;
            } else {
                throw "Property autoOpen must be a positive integer.";
            }
        }

        if (config.hasOwnProperty('modal')) {
            if (typeof config.modal === 'boolean') {
                routeeModal._config.modal = config.modal;
            } else {
                throw "Property modal must be boolean.";
            }
        } else {
            routeeModal._config.modal = true;
        }

        if (config.hasOwnProperty('appendTo')) {
            if (typeof config.appendTo === 'string' && document.querySelector(config.appendTo) !== null) {
                routeeModal._config.appendTo = config.appendTo;
            } else {
                throw "Property appendTo must be a valid \"querySelector\" string for an existing element.";
            }
        } else {
            routeeModal._config.appendTo = 'body';
        }

        routeeModal._setPosition(config);

        if (config.hasOwnProperty('animation')) {
            if (routeeModal._objectHasValue(Enums.ANIMATION_TYPE, config.animation)) {
                routeeModal._config.animation = config.animation;
            } else {
                throw "Property animation must be an ANIMATION_TYPE Enum";
            }
        } else {
            routeeModal._config.animation = Enums.ANIMATION_TYPE.fade;
        }

        if (config.hasOwnProperty('width')) {
            if (routeeModal._checkValidCssValue('width', config.width)) {
                routeeModal._config.width = config.width;
            } else {
                throw "Property width must be a valid css width";
            }
        } else {
            routeeModal._config.width = '500px';
        }

        if (config.hasOwnProperty('height')) {
            if (routeeModal._checkValidCssValue('height', config.height)) {
                routeeModal._config.height = config.height;
            } else {
                throw "Property height must be a valid css height";
            }
        } else {
            routeeModal._config.height = '300px';
        }

        if (config.hasOwnProperty('header')) {
            if (typeof config.header === 'string') {
                routeeModal._config.header = config.header;
            } else {
                throw "Property header must be a string";
            }
        }

        if (config.hasOwnProperty('body')) {
            if (typeof config.body === 'string' || typeof config.body === 'object') {
                routeeModal._config.body = config.body;
            } else {
                throw "Property body must be a string or an HTML Element";
            }
        }

        if (config.hasOwnProperty('footer')) {
            if (typeof config.footer === 'string' || typeof config.footer === 'object') {
                routeeModal._config.footer = config.footer;
            } else {
                throw "Property footer must be a string or an HTML Element";
            }
        }

        if (config.hasOwnProperty('type')) {
            if (routeeModal._objectHasValue(Enums.CLOSE_TYPE, config.type)) {
                routeeModal._config.type = config.type;
            } else {
                throw "Property type must be CLOSE_TYPE enum";
            }
        } else {
            routeeModal._config.type = Enums.CLOSE_TYPE.simple;
        }
    }
    ...
    /**
     * Set the position in the config
     *
     * @param {object} config
     * @private
     */
    _setPosition(config) {
        ...
        if (config.hasOwnProperty('position')) {
            if (config.position.hasOwnProperty('horizontal') && config.position.hasOwnProperty('vertical')) {
                if (routeeModal._objectHasValue(Enums.HORIZONTAL_POSITION, config.position.horizontal) &&
                    routeeModal._objectHasValue(Enums.VERTICAL_POSITION, config.position.vertical)
                ) {

                } else {
                    throw "Properties position.horizontal and position.vertical must be POSITION enums"
                }
            } else {
                throw "Properties position.horizontal and position.vertical are required"
            }
        } else {
            config.position = {
                horizontal: Enums.HORIZONTAL_POSITION.center,
                vertical: Enums.VERTICAL_POSITION.middle
            }
        }

        if (config.position.hasOwnProperty('horizontalMargin')) {
            if (!routeeModal._checkValidCssValue('top', position.horizontalMargin)) {
                throw 'Property position.horizontalMargin must a valid css margin';
            }
        } else {
            config.position.horizontalMargin = '20px';
        }

        if (config.position.hasOwnProperty('verticalMargin')) {
            if (!routeeModal._checkValidCssValue('top', position.verticalMargin)) {
                throw 'Property position.verticalMargin must a valid css margin';
            }
        } else {
            config.position.verticalMargin = '20px';
        }

        routeeModal._config.position = {
            horizontal: {},
            vertical: {},

        };

        routeeModal._config.position.userDefined = config.position;

        switch (config.position.horizontal) {
            case Enums.HORIZONTAL_POSITION.left:
                routeeModal._config.position.horizontal.property = 'left';
                routeeModal._config.position.horizontal.value = config.position.horizontalMargin;
                break;
            case Enums.HORIZONTAL_POSITION.center:
                routeeModal._config.position.horizontal.property = 'right';
                routeeModal._config.position.horizontal.value = '50%';
                break;
            case Enums.HORIZONTAL_POSITION.right:
                routeeModal._config.position.horizontal.property = 'right';
                routeeModal._config.position.horizontal.value = config.position.horizontalMargin;
                break;
        }
        ...
        switch (config.position.vertical) {
            case Enums.VERTICAL_POSITION.top:
                routeeModal._config.position.vertical.property = 'top';
                routeeModal._config.position.vertical.value = config.position.verticalMargin;
                break;
            case Enums.VERTICAL_POSITION.middle:
                routeeModal._config.position.vertical.property = 'bottom';
                routeeModal._config.position.vertical.value = '50%';
                break;
            case Enums.VERTICAL_POSITION.bottom:
                routeeModal._config.position.vertical.property = 'bottom';
                routeeModal._config.position.vertical.value = config.position.verticalMargin;
                break;
        }

        if (config.position.vertical === Enums.VERTICAL_POSITION.middle &&
            config.position.horizontal === Enums.HORIZONTAL_POSITION.center
        ) {
            routeeModal._config.position.transform = 'translate(50%, 50%)';
        } else if(config.position.vertical === Enums.VERTICAL_POSITION.middle) {
            routeeModal._config.position.transform = 'translateY(50%)';
        } else if (config.position.horizontal === Enums.HORIZONTAL_POSITION.center) {
            routeeModal._config.position.transform = 'translateX(50%)';
        }
    }
    ...
    _render() {
        let routeeModal = this;
        let builder = new ModalBuilder(routeeModal._config);
        routeeModal.element = builder.getModal();

        if (routeeModal._config.animation === Enums.ANIMATION_TYPE.fade ||
            routeeModal._config.animation === Enums.ANIMATION_TYPE.none
        ) {
            routeeModal._setConfigPosition();
        } else {
            routeeModal._setClosedPosition();
        }

        let popupMainElement = routeeModal.element.getElementsByClassName(Constants.POPUP.main)[0];
        popupMainElement.style.setProperty('opacity', '0');

        let containerElement = document.querySelector(routeeModal._config.appendTo);
        containerElement.appendChild(routeeModal.element);

        let closeElements = document.getElementsByClassName('routee-modal-close');
    ...
    /**
     * Set popup position as defined in the config
     *
     * @private
     */
    _setConfigPosition() {
        let routeeModal = this;
        let modalElement = routeeModal.element.getElementsByClassName(Constants.POPUP.main)[0];
        const horizontalProperty = routeeModal._config.position.horizontal.property;
        const verticalProperty = routeeModal._config.position.vertical.property;
        const horizontalValue = routeeModal._config.position.horizontal.value;
        const verticalValue = routeeModal._config.position.vertical.value;
        modalElement.style.setProperty(horizontalProperty, horizontalValue);
        modalElement.style.setProperty(verticalProperty, verticalValue);

        if (routeeModal._config.position.hasOwnProperty('transform')) {
            modalElement.style.setProperty('-webkit-transform', routeeModal._config.position.transform);
            modalElement.style.setProperty('-moz-transform', routeeModal._config.position.transform);
            modalElement.style.setProperty('transform', routeeModal._config.position.transform);
        }
    }

    /**
     * Set popup closed position
     *
     * @rivate
     */
    _setClosedPosition() {
        let routeeModal = this;
        let position = routeeModal._config.position;
        let modalElement = routeeModal.element.getElementsByClassName(Constants.POPUP.main)[0];

        switch (routeeModal._config.animation) {
            case Enums.ANIMATION_TYPE.slideUp:
                if (position.userDefined.vertical === Enums.VERTICAL_POSITION.middle) {
                    modalElement.style.setProperty('bottom','0px')
                } else {
                    let sign = (position.vertical.property === 'top')? '+' : '-';
                    modalElement.style.setProperty(position.vertical.property,`calc(${position.vertical.value} ${sign} 500px)`)
                }
                break;
            case Enums.ANIMATION_TYPE.slideDown:
                if (position.userDefined.vertical === Enums.VERTICAL_POSITION.middle) {
                    modalElement.style.setProperty('bottom','100%')
                } else {
                    let sign = (position.vertical.property === 'bottom')? '+' : '-';
                    modalElement.style.setProperty(position.vertical.property,`calc(${position.vertical.value} ${sign} 500px)`)
                }
                break;
            case Enums.ANIMATION_TYPE.slideRight:
                if (position.userDefined.horizontal === Enums.HORIZONTAL_POSITION.center) {
                    modalElement.style.setProperty('right','0px')
                } else {
                    let sign = (position.horizontal.property === 'right')? '+' : '-';
                    modalElement.style.setProperty(position.horizontal.property,`calc(${position.horizontal.value} ${sign} 500px)`)
                }
                break;
            case Enums.ANIMATION_TYPE.slideLeft:
                if (position.userDefined.horizontal === Enums.HORIZONTAL_POSITION.center) {
                    modalElement.style.setProperty('right','100%')
                } else {
                    let sign = (position.horizontal.property === 'left')? '+' : '-';
                    modalElement.style.setProperty(position.horizontal.property,`calc(${position.horizontal.value} ${sign} 500px)`)
                }
                break;
        }
    ...
    _modalToggle() {
        let routeeModal = this;
        let modalElement = routeeModal.element.getElementsByClassName(Constants.POPUP.main)[0];
        let newOpacity = routeeModal.isOpen ? '0' : '1';
        modalElement.style.setProperty('opacity', newOpacity);

        if (routeeModal.isOpen) {
            routeeModal._setClosedPosition();
        } else {
            routeeModal._setConfigPosition();
        }
    ...
    /**
     * Check if a value is valid for a specific CSS property
     *
     ...
     * @private
     */
    _checkValidCssValue(property, value) {
...
// CONCATENATED MODULE: ./node_modules/routeemodal/src/index.js





// CONCATENATED MODULE: ./src/js/PushConditionParser.js



class PushConditionParser_PushConditionParser {

    /**
     * PushConditionParser constructor
     *
     * @param {object} conditions
     * @param {string} conditions.type_1
     * @param {string|object} conditions.action_1
     * @param {string} conditions.type_2
     * @param {string|object} conditions.action_2
     * @param {string} conditions.operator
     */
    ...
    /**
     * Check if conditions are met to show push opt in dialog
     *
     * @return {boolean}
     */
    canShowDialog() {
        let thisClass = this;
        return thisClass._parseConditions(thisClass._conditions);
    }
    ...
    getInterval() {
        let thisClass = this;
        let pageVisitsAction = thisClass._getCondition(constants.CONDITIONS.TYPES.pageVisits);
        let intDelay = 0;

        if (
            thisClass._conditions.operator === constants.CONDITIONS.OPERATORS.or ||
            thisClass._checkCondition(constants.CONDITIONS.TYPES.pageVisits, pageVisitsAction)
        ) {
            const delay = parseInt(thisClass._getCondition(constants.CONDITIONS.TYPES.delay));
            const scriptDelay = Math.floor((new Date().getTime() - constants.VISIT_TIMESTAMP) / 1000);
            intDelay = delay - scriptDelay;
        }
        return intDelay;
        // TODO: update logic when more condition types are implemented
    }
    ...
    _getCondition(type) {
        ...
        if (thisClass._conditions.type_1 === type) {
            return thisClass._conditions.action_1
        }

        if (thisClass._conditions.type_2 === type) {
            return thisClass._conditions.action_2
        }

        return null;

        // TODO: update logic when type = 'condition' is implemented
    }
     ...
     * @private
     */
    _parseConditions(condition, count = 0) {
        let thisClass = this;
        ...
        if (count < 100) {
            ...
            if (condition.type_1 === 'condition' && typeof condition.action_1 === 'object') {
                condOne = thisClass._parseConditions(condition.action_1, count++);
            } else {
                condOne = thisClass._checkCondition(condition.type_1, condition.action_1)
            }

            if (condition.type_2 === 'condition' && typeof condition.action_2 === 'object') {
                condTwo = thisClass._parseConditions(condition.action_2, count++);
            } else {
                condTwo = thisClass._checkCondition(condition.type_2, condition.action_2)
            }

            switch (condition.operator) {
                case constants.CONDITIONS.OPERATORS.and:
                    return condOne && condTwo;
                case constants.CONDITIONS.OPERATORS.or:
                    return condOne || condTwo;
            }
        }
        return false;
     ...
     * @private
     */
    _checkCondition(type, action) {
        let conditionsMet = false;

        switch (type) {
            case "pageVisits":
                conditionsMet = Cookie_Cookie.getSessionCount() >= parseInt(action);
                break;
            case "delay":
                conditionsMet = (new Date().getTime() - constants.VISIT_TIMESTAMP) >= (parseInt(action) * 1000);
                break;
        }
        return conditionsMet;
    }
}
// CONCATENATED MODULE: ./src/js/PushNotification.js
...
class PushNotification_PushNotification {

    /**
     * PushNotification constructor
     *
     * @param {object} configuration - firebase config
     * @param {object} configuration.dialogConfig
     * @param {number} configuration.dialogConfig.later
     * @param {string} configuration.dialogConfig.message
     * @param {string} configuration.dialogConfig.allow
     * @param {string} configuration.dialogConfig.disallow
     * @param {object} configuration.dialogConfig.conditions
     * @param {HTMLElement} popupElement
     * @param {function} optInCallback
     */
    constructor(configuration, popupElement, optInCallback) {
        let thisClass = this;

        let dialogConfig = {...configuration.dialogConfig};
        delete configuration.dialogConfig;

        thisClass._config = {...configuration};
        thisClass._dialog = dialogConfig;
        thisClass._errors = [];
        thisClass._optInCallback = optInCallback;
        thisClass._popUpElement = popupElement;
    }
    ...
    async init() {
        ...
        index_cjs_default.a.initializeApp(thisClass._config);
        index_cjs_default.a.analytics();
        thisClass._messaging = index_cjs_default.a.messaging();

        /**
         * Set up push notification service worker
         */
        if ('serviceWorker' in navigator) {
            let registration = await thisClass._registerSw().catch(function (error) {
                thisClass._setError("Service worker registration", error);
            });

            thisClass._messaging.useServiceWorker(registration);
        } else {
            thisClass._setError("Old browser", "Service worker not in navigator")
        }

        /**
         * Update push notification token when tokenRefresh event fired
         */
        thisClass._messaging.onTokenRefresh(function () {
            thisClass._messaging.getToken().then(function (refreshedToken) {
                thisClass._optInCallback({token: refreshedToken, permission: constants.PUSH_PERMISSIONS.granted});
            }).catch(function (err) {
                thisClass._setError('Routee error: Unable to retrieve refreshed token ', err);
            });
        });
         ...
         */
        thisClass._messaging.onMessage(function (payload) {
            /* let customCloseFlag = (typeof payload.data.timeOnScreen !== 'undefined');
            ...
            if (customCloseFlag) {
                setTimeout(function () {
                    notification.close();
                }, parseInt(payload.data.timeOnScreen) * 1000);
            } */
        });

        //Create popup dialog for push notification opt-in
        thisClass._createOptinDialog();
        
        setInterval(function() {
            if (Notification.permission.toLowerCase() === constants.PUSH_PERMISSIONS.denied && Cookie_Cookie.get(constants.COOKIES.later) === 'true') {
                console.log('y');
            }
        }, 1000);

        // Check of push opt in in not already granted and then show dialog
        if (thisClass._checkNotificationPermission()) {
            thisClass._setDialogAutoOpen();
        } else {
            thisClass._savePushToken();
        }
    }

    /**
     * Register a service worker to firebase
     *
    ...
    _registerSw() {
        return new Promise(function (resolve, reject) {
            let swUrl = ('SERVICE_WORKER' in constants) ? constants.SERVICE_WORKER : constants.URL.sw;
    ...
    _createOptinDialog() {
        ...
        let allowButtons = thisClass._popUpElement.getElementsByClassName(constants.CSS.allowBtnCls);
        let laterButtons = thisClass._popUpElement.getElementsByClassName(constants.CSS.disallowBtnCls);

        for (let btn of allowButtons) {
            btn.addEventListener('click', function () {
                thisClass._pushDialog.close();
                thisClass._pushNotificationPermission(true);
            })
        }

        for (let btn of laterButtons) {
            btn.addEventListener('click', function () {
                thisClass._pushDialog.close();
                thisClass._pushNotificationPermission(false);
            })
        }

        thisClass._pushDialog = new RouteeModal({
            modal: false,
            position: {
                horizontal: Enums.HORIZONTAL_POSITION.center,
                vertical: Enums.VERTICAL_POSITION.top
            },
            body: thisClass._popUpElement,
            width: '400px',
            height: 'auto',
            type: Enums.CLOSE_TYPE.none
        });
    }

    /**
     * PushNotification error callback.
     *
    ...
    /**
     * PushNotification event that triggers when there is an error
     *
    ...
    /**
     * Run push notification opt-in dialog
     */
    openDialog() {
    ...
    /**
     * Check if can show notification opt in dialog
     *
     * @return {boolean}
     * @private
     */
    _checkNotificationPermission() {
        ...
        return permission !== constants.PUSH_PERMISSIONS.granted && !Cookie_Cookie.get(constants.COOKIES.later);
    }
    ...
    _savePushToken() {
        ...
        if (permission === constants.PUSH_PERMISSIONS.granted) {
            thisClass._messaging.getToken().then(function (currentToken) {
                thisClass._optInCallback({token: currentToken, permission: constants.PUSH_PERMISSIONS.granted});
            }).catch(function (error) {
                thisClass._optInCallback({permission: 'error'});
                thisClass._setError('Routee Error: Failed to retrieve token. ', error);
            });
        }
    }

    /**
     * Run loop to check if conditions met to show push opt in dialog
     *
     * @private
     */
    _setDialogAutoOpen() {
        ...
        if ('conditions' in thisClass._dialog) {
            let conditionParser = new PushConditionParser_PushConditionParser(thisClass._dialog.conditions);
            const seconds = conditionParser.getInterval();

            thisClass._dialogInterval = setInterval(() => {
                if (conditionParser.canShowDialog()) {
                    thisClass.openDialog();
                    clearInterval(thisClass._dialogInterval);
                } else if (seconds <= 0) {
                    clearInterval(thisClass._dialogInterval);
                }
            }, seconds);
    ...
    /**
     * Push notification dialog opt-in action
     *
     * @param {boolean} flag
     * @private
     */
    _pushNotificationPermission(flag) {
        let thisClass = this;
        let permission = Notification.permission.toLowerCase();

        if (flag) {
            if (permission === constants.PUSH_PERMISSIONS.default) {
                Notification.requestPermission().then(function (newPermission) {
                    if (newPermission === constants.PUSH_PERMISSIONS.granted) {
                        thisClass._savePushToken();
                        // Click allow on both popups
                    } else {
                        thisClass._optInCallback({permission: newPermission});
                        Cookie_Cookie.set(constants.COOKIES.later, "true", parseInt(thisClass._dialog.later));
                        // Clicked allow on 1st popup but didnt click allow on second
                    }
                });
            } else {
                thisClass._optInCallback({permission: permission});
                Cookie_Cookie.set(constants.COOKIES.later, "true", parseInt(thisClass._dialog.later));
                // Clicked allow on first popup but is blocked in the browser
                // TODO: show guide how to manually opt-in from the browser
            }
        } else {
            thisClass._optInCallback({permission: permission});
            Cookie_Cookie.set(constants.COOKIES.later, "true", parseInt(thisClass._dialog.later));
            // Clicked later on 1st popup
        }
    }
}

// CONCATENATED MODULE: ./src/js/routeeV2.js
...
/**
 * Creates cookies and gets configurations
 *
 * @param data
 * @return {Promise<void>}
 */
routeeV2_routee.initialize = async function (data) {
    constants.VISIT_TIMESTAMP = new Date().getTime();

    // Check if routeeToken (UUID) was provided
    if ('routeeToken' in data) {
        constants.ROUTEE_TOKEN = data.routeeToken;
    } else {
        throw "RouteeToken not provided";
    }

    // Check if custom URL for service worker was provided
    if ('serviceWorker' in data) {
        constants.SERVICE_WORKER = data.serviceWorker;
    }

    routeeV2_routee.errors = [];

    // Get Cookies
    let configurationRequest = getConfig(data.routeeToken);
    let analyticsId = Cookie_Cookie.get(constants.COOKIES.analytics);
    let trackingId = Cookie_Cookie.get(constants.COOKIES.tracking);
    let session = Cookie_Cookie.getSession();

    // Set COOKIES
    if (trackingId === null) {
        let frameRequest = createFrame();

        trackingId = await createSession().catch((error) => {
            setError('Session Error', error);
        });

        if (typeof trackingId !== 'undefined') {
            Cookie_Cookie.set(constants.COOKIES.tracking, trackingId);
            Cookie_Cookie.setSession();

            if (analyticsId === null) {
                frameRequest.then((aId) => {
                    Cookie_Cookie.set(constants.COOKIES.analytics, aId);
                    updateSession(trackingId);
                }).catch((error) => {
                    setError('AnalyticsId Error', error);
                });
            } else {
                Cookie_Cookie.set(constants.COOKIES.analytics, analyticsId);
                updateSession(trackingId);
            }
        } else {
            trackingId = null;
        ...
        if (!session) {
            updateSession(trackingId).then(() => {
                Cookie_Cookie.setSession();
            }).catch((error) => {
                setError('Session Update Error', error);
            });
        }

        // If analyticsId cookie doesn't exist create it
        if (analyticsId === null) {
            createFrame().then((analyticsId) => {
                Cookie_Cookie.set(constants.COOKIES.analytics, analyticsId);
                updateSession(trackingId);
            }).catch((error) => {
                setError('AnalyticsId Error', error);
    ...
    /* Check if main COOKIE was created and run main code */
    if (trackingId !== null) {
        let configuration = await configurationRequest.catch((error) => {
            setError('Configuration Error', error);
        });
...
function runConfig(configuration, trackingId) {
    ...
    if ('pushConfig' in configuration) {
        ...
        let pushConfig = {...configuration.pushConfig};
        let dialogConfig = {...configuration.pushConfig.dialogConfig};
        ...
        const pushOptInCallBack = (optInData) => {
            ...
            optInData.routeeToken = constants.ROUTEE_TOKEN;

            if (analyticsId !== null) {
                optInData.analyticsId = analyticsId;
            }
            savePushOptIn(optInData).catch((error) => {
                setError("Save Push Token", error)
            });
        };

        /* Create a "push notification service" object */
        let pushNotification = new PushNotification_PushNotification(pushConfig, popupHtml, pushOptInCallBack);

        /* Catch push notification errors */
        pushNotification.onError((error) => {
            routeeV2_routee.errors.push(error);
        });

        /* Initialize and run push notification service  */
        pushNotification.init().catch((error) => {
            setError("Push init", error);
        });
    } else {
        setError("No push config", configuration);
    }
}
...
/**
 * Save browser information of a visitor and creates a UUID to represent the visitor
 *
 * @return {Promise<string>} trackingId
 */
function createSession() {
    let data = Utils_Utils.getBrowserData();
    data.routeeToken = constants.ROUTEE_TOKEN;

    return new Promise((resolve, reject) => {
        let xhr = new XMLHttpRequest();
        xhr.open('POST', constants.URL.session);
...
/**
 * Renew the browser data of a visitor
 *
 * @param trackingId
 * @return {Promise<XMLHttpRequest response>}
 */
function updateSession(trackingId) {
    let data = Utils_Utils.getBrowserData();
    data.routeeToken = constants.ROUTEE_TOKEN;

    return new Promise((resolve, reject) => {
        let xhr = new XMLHttpRequest();
        xhr.open('PUT', constants.URL.session + '/' + trackingId);
        xhr.responseType = 'json';
        xhr.setRequestHeader('Content-Type', 'application/json');
...
/**
 * Get all the configurations of a routeeToken
 *
 * @param {string} uuid - routeeToken
 * @return {Promise<object>}
 */
function getConfig(uuid) {
    return new Promise((resolve, reject) => {
        let xhr = new XMLHttpRequest();
        xhr.open('GET', constants.URL.config + '/' + uuid);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.responseType = 'json';
...
/**
 * Save an FCM token of a visitor
 *
 * @param data
 * @return {Promise<XMLHttpRequest response>}
 */
function savePushOptIn(data) {
    return new Promise((resolve, reject) => {
        let xhr = new XMLHttpRequest();
        xhr.open('POST', constants.URL.optin);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.responseType = 'json';
 ...
 *
 * @param {number} [timeout=30] - Promise timeout in seconds
 * @return {Promise<string>}
 */
function createFrame(timeout = 30) {
    let frameElement = document.createElement('iframe');
    frameElement.src = constants.URL.frame;
    ...
    return new Promise((resolve, reject) => {
        ...
        window.addEventListener('message', (event) => {
            if (event.origin === constants.URL.base && event.data.hasOwnProperty('analyticsId')) {
                clearInterval(timeoutInterval);
                frameElement.remove();
                resolve(event.data.analyticsId);
            }
        }, false);
...
function getDialogHTML(title, message, allow, disallow, icon = null) {
    let pushContainerElement = document.createElement('div');
    ...
    let laterBtnElement = document.createElement('div');
    ...
    let allowBtnElement = document.createElement('div');
